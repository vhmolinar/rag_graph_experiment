...
| AC-11 | Comparativa não usa uma obra só sem declarar limitação | ✅ | T10/T12 cobrem diversidade adaptativa sem quota cega; T13: `test_dissertative_pipeline.py::test_comparative_with_single_work_declares_limitation` e `test_dissertative.py::TestComparativeLimitation` cobrem declaração determinística com fonte única; R05: `limitations` preservadas em `AnswerRun` (migration `0011`), persistência testada em `test_repositories.py::test_run_roundtrip_with_limitations`, projetadas no envelope `QueryState` (`test_api_schemas.py::test_build_query_state_preserves_limitations`), e entregues via GET `/queries/{id}` e SSE terminal (`test_api.py::test_comparative_query_single_work_returns_limitation_via_get_and_sse` e `test_api.py::test_comparative_query_two_works_does_not_declare_single_source_limitation`). |
